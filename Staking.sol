...
contract StakingRewards is Ownable {
    ...
    function calculateRewards() public view returns (uint _rewards) {
        ...
        uint updateBlockQueue = rewardChanges.length - 1;
        uint startingBlock = block.number;

        uint _lastUpdatedBlock = lastUpdatedBlock[msg.sender];

        if (rewardChanges[updateBlockQueue].block > _lastUpdatedBlock) {
        ...
        return _rewards;
            ...
            while (rewardChanges[updateBlockQueue].block > _lastUpdatedBlock) {
                _rewards += _mouseBalance * (startingBlock - rewardChanges[updateBlockQueue].block) * rewardChanges[updateBlockQueue].rewardPerBlock;
                startingBlock = rewardChanges[updateBlockQueue].block;
                updateBlockQueue -= 1;
            }
        return _rewards;
        }
